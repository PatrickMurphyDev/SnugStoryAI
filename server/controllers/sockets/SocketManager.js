...
class SocketManager {
  ...
  setupOllama() {
    ...
    this.ollama.setSystemPrompt("#Your task is to roleplay as the supplied NPC character, "
...
async storeGameState(saveGameID, gameState) {
  ...
  buildPlotPromptTXT(data) {
    let sendMsg = " Subtly include: " + this.overallNarrative.intro[Math.floor(this.overallNarrative.intro.length * Math.random())] + ". ";
    return sendMsg;
  }
...
buildAIPromptTXT(data, dataPrefix, userData) {
    ...
    let sendMsg = "# Respond as the character " + this.GetCharacterData(data.to).name; + ".";
    let that = this;
